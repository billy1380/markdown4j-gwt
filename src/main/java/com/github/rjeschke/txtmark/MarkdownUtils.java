...
public class MarkdownUtils {
	...
	public final static void appendMailto(final StringBuilder out, final String in, final int start, final int end) {
		for (int i = start; i < end; i++) {
			...
			case '\'':
			case '@':
				if (r < 512) appendDecEntity(out, c);
				else appendHexEntity(out, c);
				break;
			default:
				if (r < 32) out.append(c);
				else if (r < 520) appendDecEntity(out, c);
				else appendHexEntity(out, c);
				break;
			}
		}
	...
	public final static String getMetaFromFence(String fenceLine) {
		for (int i = 0; i < fenceLine.length(); i++) {
			...
			if (!Character.isSpace(c) && c != '`' && c != '~' && c != '%') { return fenceLine.substring(i).trim(); }
		}
		return "";
	}
	...
	public static Map<String, String> parseParams(String s) {
		...
		while ((m = regExp.exec(s)) != null) {
			params.put(m.getGroup(1), m.getGroup(2));
		}
