...
class Emitter {
	...
	private int findToken(final String in, int start, MarkToken token) {
		int pos = start;
		while (pos < in.length()) {
			if (this.getToken(in, pos) == token) return pos;
			pos++;
		}
		return -1;
	}
	...
	private MarkToken getToken(final String in, final int pos) {
		...
		switch (c) {
		case '*':
			if (c1 == '*') { return c0 != ' ' || c2 != ' ' ? MarkToken.STRONG_STAR : MarkToken.EM_STAR; }
			return c0 != ' ' || c1 != ' ' ? MarkToken.EM_STAR : MarkToken.NONE;
		case '_':
			if (c1 == '_') { return c0 != ' ' || c2 != ' ' ? MarkToken.STRONG_UNDERSCORE : MarkToken.EM_UNDERSCORE; }
			if (this.useExtensions) { return Character.isLetterOrDigit(c0) && c0 != '_' && Character.isLetterOrDigit(c1) ? MarkToken.NONE
					: MarkToken.EM_UNDERSCORE; }
			return c0 != ' ' || c1 != ' ' ? MarkToken.EM_UNDERSCORE : MarkToken.NONE;
		case '~':
			if (this.useExtensions && c1 == '~') { return MarkToken.STRIKE; }
		...
		default:
			if (this.useExtensions) {
				switch (c) {
				case '-':
					if (c1 == '-') return c2 == '-' ? MarkToken.X_MDASH : MarkToken.X_NDASH;
					break;
				case '^':
				...
				case '(':
					...
					break;
				case '"':
					if (!Character.isLetterOrDigit(c0) && c1 != ' ') return MarkToken.X_LDQUO;
					if (c0 != ' ' && !Character.isLetterOrDigit(c1)) return MarkToken.X_RDQUO;
					break;
				case ':':
					return c0 == ':' || c1 == ':' ? MarkToken.NONE : MarkToken.EMOJI;
				}
			}
			return MarkToken.NONE;
		}
	...
	private void emitMarkedLines(final StringBuilder out, final Line lines) {
		...
		while (line != null) {
			if (!line.isEmpty) {
				...
				if (line.trailing >= 2 && !convertNewline2Br) in.append("<br />");
			}
			if (line.next != null) {
				in.append('\n');
	...
	private void emitCodeLines(final StringBuilder out, final Line lines, final String meta, final boolean removeIndent) {
		...
		if (this.config.codeBlockEmitter != null) {
			...
			while (line != null) {
				if (line.isEmpty) list.add("");
				else list.add(removeIndent ? line.value.substring(4) : line.value);
				line = line.next;
			}
